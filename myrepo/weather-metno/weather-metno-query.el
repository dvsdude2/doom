;;; weather-metno-query.el --- Query language for weather-metno-el -*- lexical-binding: nil -*-
...
(defun weather-metno--time-in-range-p (from to from2 to2)
  "Check if time range FROM TO overlaps with time range FROM2 TO2.
Return non-nil if there is any overlap between the two time ranges.
The time ranges are inclusive at the start and exclusive at the end,
meaning FROM and FROM2 are included in their respective ranges, while
TO and TO2 are not."
...
(defun weather-metno--op-select (ops entry &optional other)
...
(defmacro weather-metno-query-by-hour (x &rest body)
  "Queries DATA for values at LOCATION for DATE, taking time of day into acccount.
