...
(after! dirvish
  (setopt dirvish-hide-details '(dired dirvish dirvish-side))
...
(map! :after org
      ...
      :n "c b" #'org-insert-structure-template
      :desc "break/split code block"
      :n "c B" #'org-babel-demarcate-block)

(map! :after dired
      :map dired-mode-map
      :leader
      :desc "append to file"
      :n "f a" #'append-to-file
      :desc "open all marked files at once"
      :n "f m" #'dired-do-find-marked-files)
...
(map! :leader
      ...
      :n    "o a f" #'org-cycle-agenda-files
      :desc "open calendar"
      :n    "o c" #'=calendar
      :desc "open Dashboard"
      :n    "o D" #'dashboard-open
      :desc "open defalt scratch-buffer"
      :n    "o x" #'scratch-buffer
      :desc "run org-drill on drill file"
      :n    "o l" #'my/org-drill
      :desc "open org config in workspace"
      :n    "o I" #'=config
      :desc "open org config"
      :n    "o i" (lambda () (interactive) (find-file "~/.config/doom/config.org"))
      :desc "open org notes"
      :n    "o n" (lambda () (interactive) (find-file "~/org/notes.org"))
      :desc "open org organizer"
      :n    "o 0" (lambda () (interactive) (find-file "~/org/organizer.org"))
...
(map! :leader
      ...
      :desc "vertico tray menu"
      :n "v" #'tray-vertico-menu))
